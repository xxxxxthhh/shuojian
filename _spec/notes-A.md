...
把重力放在起跳之后会让顶点变成 **102**（少 12px），二段跳手感也会跟着变。

> **这一节不再只是文档。** `dev/check.html` 里有一项「手感基线」深检：
> `SJ.Player` 一存在，它就会自动建一个平地世界、用合成按键把 Player 跑一遍，
> 比对 **长按顶点 114±4 / 点按顶点 52.3±6 / 走速 240±3 / 土狼窗口**。
>
> 土狼那项刻意做成**对设计中立**：量的是「走出平台边缘后第 N 帧起跳能升多高」，
> 而不是「跳没跳」。窗口内（4 帧）必须拿到**满跳**（≥ 实测长按顶点 -6px）；
> 窗口外（12 帧）不许再拿到满跳。这样一来，Player 若允许「走下平台后仍可空中二段跳」
> （-640，约升 90px）不会被误判成土狼时间过长——**那是合理的设计选择，不该由这道闸门否决**。
>
> 量不出来（Player 依赖的模块没就绪）报**黄**不报红；页面会给缺席模块临时装空壳
> （只补缺的，绝不覆盖真模块，量完立刻拆），所以 fx.js 没落地也能照量。
> 已用替身 Player 验证三种情形：正确的报绿；重力顺序写反的报红（实测 102.0）；走速写成 200 的报红。
...
| `dev/check.html` | **契约自检闸门**。按 index.html 顺序加载全部脚本（缺的会 404，页面不崩），逐个查 `SJ.*` 存在性与 typeof，分模块绿/红/灰列出，控制台打印缺失清单。另有「深检」：Levels 时长预算 ≥1800、7 种杂兵、12 招、Figure 必备姿势、Script 入口 key，以及 **§4 手感基线自动比对**（见下）。每波结束跑。 |
...
## 6. playtimeSec（决议 006，已实现）

唯一写入者在 `Game.step()` 里，**每帧一处**：

```js
if (G.scene && G.scene.countsPlaytime !== false) SJ.Save.data.playtimeSec += STEP;
```

- **用的是 `rawDt`（恒 1/60）不是 `dt`。** 这是关键：用 `dt` 的话 0.35× 观势会把
  真实的 2 秒记成 0.7 秒，而这个数字正是用来验收 DESIGN §9.6「单周目 30 分钟以上」的。
- **`G.scene` 是栈顶**，所以对话/剧情叠加层压在关卡上时**照常计时**（188 个剧本节点占时不小）。
- **`countsPlaytime !== false` 是默认计时**，只有显式写 `countsPlaytime = false` 才停表。

### 给 H 的一句话
**标题 / 暂停 / 招式槽 / 结算 这几个 scene 请设 `countsPlaytime = false`。**
```js
var pauseScene = { countsPlaytime: false, enter: …, update: …, draw: … };
```
剧情与对话层**不要**设（它们应当计时）。

已测 6 项：60 帧=1.000s；hitstop 冻结 `Game.time` 但 playtime 照走；0.35× 慢镜记 2.0000s
（误用 dt 会是 0.70）；`countsPlaytime:false` 记 0；暂停 push 3 秒后 pop，总数仍是 2.0000s。
