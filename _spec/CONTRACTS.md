...
## 决议 022 — 血量：死亡复活与章节开头一律回满（T1 发现，Lead 裁定）

现状：`Save.data.hp` 只在 `Level.complete()` 写一次，检查点复活与下一章开头都读它；全游戏没有回血手段。
后果（T1 实测 seed=7）：第一回 2 血通关 → 第二回每次复活 2 血、死 16 次 → 第三回死 54 次打不完。这是死亡螺旋，玩家会以为游戏坏了。
DESIGN §8 没定过这条规则，现在定：
- **死亡从检查点复活：满血。**
- **进入新章节：满血。** `Level.complete()` 写 `hp = maxHp`（或不写），`restartFromCheckpoint` 载入前置满。
- 「继」（中途续档）读到的自然就是满血。墨 `ink` 的规则不动。
实现全在 `src/level/level.js`（T1），不碰 player.js / save.js。level-check 或 respawn-audit 加断言：任意关卡死亡复活后 `hp === maxHp`。

## 决议 023 — 无门波次不得阻塞后续波次（T1 发现，Lead 裁定）

`updateWaves` 一次只跑一波；无 gate 的波次可被绕过，其敌人只要活着，后面的波次就永远不刷（第五回第 2 波挡住 Boss 前第 3 波，内容静默消失）。
定：**无 gate 的波次是「非阻塞」的**——玩家触到后一波的触发区时，仍活跃的无门波次视为结束（敌人留在原地当散兵，不再计入门锁）。有 gate 的波次语义不变。
level.js 实现 + level-check 加规则「无门波次之后若有波次，运行时不得被其阻塞」（用 harness 真跑一次验证，而不是静态查）。

## 决议 024 — 「悟」时槽位已满：在悟的画面里选替换（T1 发现，Lead 裁定）

`Tech.learn` 只往空槽塞；一周目到第五回时四槽通常已满，守阁人**只有无锋能撬开**，于是「学会了装不上 → Boss 完全无敌」，
游戏没有任何提示要去暂停菜单换招（DESIGN §0 铁律 3 不做教程弹窗）。这是全游戏唯一「不换招就打不过」的 Boss，比第一回 Boss 硬得多。
定：**「悟」的画面（learnScene）在无空槽时展示四个槽位 + 新招**，←/→ 选替换哪个（缺省最后一槽），J 确认；
1.05s 后可跳过 = 按缺省替换。无文字提示，与生杀选择同一种语言。有空槽时行为不变。
实现：`src/combat/tech.js`（T2）。走契约 007 §1 的 `SJ.Player.setSlot` 唯一写入口。player-check 加断言：四槽满时学新招后新招必在槽内。

## 决议 025 — 预警分层的三处细调（T3 动态验证，Lead 裁定）

1. **heavy 势点提前长出来**：不观势时势点从起手进度 p≥0.25 就可见、按 p 线性长到满（现在末段才出现，中段三档分不开）。线宽 ×1.6 从起手第一帧就生效。—— combat.js（T2）
2. **grab 短路径**：落点双环的外环半径设下限（≥ 22px），与路径长度脱钩；孤影/闪这种 ≤110px 的路径靠双环传达。—— combat.js（T2）
3. **瞬移 burst 挪到出发点**：孤影/闪的 `FX.burst` 黑点现在堆在落点环上、与预警抢地方。改到消失处（出发点），落点只留预警。—— ai.js（T3）
均为呈现，不改判定/时机。

## 决议 026 — 第六回师兄与「贴脸按住观势」——本波不动，记录

T1 实测：师兄是全游戏最陡的墙（机器人 5 种子 1 过、死 12–14 次）；「贴身一直按 K」能把挨打压到十分之一。
公平性（T3）每招 ≥0.40s 无问题。是否调难度 = 数值，**等用户打到第六回反馈后再定**。

## 决议 027 — 门只锁同层（用户实测第二回软锁，Lead 热修）

第二回三层触发第 5 波后按下+跳下穿到一层，门在一层照样夹住 x≥1400，回三层的台阶全在门外；敌人在楼上、玩家在楼下。
T1 修过「敌人跑出楼层」（rescueFoes），这是它的镜像：玩家自己离开楼层。
定：`clampPlayer` 的波次门只在 `onSameFloor(p, R.active.def)` 为真时生效；玩家脚底离开那一层，门放人；回到那一层再夹。
单层关卡行为不变；Boss 场地（bossArena）不走这条。level-check 规则 24 用 harness 真跑：同层必锁 / 异层必放；变异测试确认去掉修复即红（c2 wave4/5、c5 wave3）。
