...
(function (SJ) {
   ...
   * ══════════════════════════════════════════════════════════════════ */
  {
    ...
    solids: [
      ...
      [600, 410, 130, 60, 1],       // 水洼里的垫脚石（第一次跳）
      [900, 410, 140, 60, 1],       // 第二块（连跳）
   ...
   *   教学摆位（STORY §5）：
   *     wall_c1_a(690)「未出鞘 先见招」题壁先垫一句
   *     c1_t_watch(880) ★决议 012：**排在 w1 触发线（790）之后** —— 顺序不能反：
   *                     先刷出刀客、让他起手，旁白再落下，玩家才是「看着那道红虚线」读这一句。
   *                     反过来（旁白在前）玩家会对着一屏空竹林读「招已经画在空里了」。
   *                     w1 的 gate 是 [780,1320]，刀客在 1150 —— 玩家绕不过他，必须打上照面。
   ...
   * ══════════════════════════════════════════════════════════════════ */
  {
    ...
    deco: [
      { kind: 'stele', x: 730, y: 470 },       // wall_c1_a 的碑
    ...
    spawns: [
      // ★ 决议 012 路径三：教具刀客只出破雨。玩家要在同一条朱砂虚线上看第二遍、第三遍，
      //   才可能自己发现「原来那条线是可以读的」。这是全游戏唯一一处钉死招表的 spawn。
      { type: 'daoke', x: 1150, y: 470, wave: 1, only: ['poyu'] },
    ...
    waves: [
      { id: 1, x: 790, w: 60, gate: [780, 1320], sec: 14, note: '一个刀客，一条起手式，看清就行。触发线特意排在 c1_t_watch(880) 之前 —— 先让他出现并起手，旁白再落下' },
    ...
    triggers: [
      { x: 690, y: 380, w: 80, h: 90, interact: true, once: true, event: { play: 'wall_c1_a' } },
      // ★ 决议 012：这一句必须和第一个刀客同框 —— 玩家读到「停的那一下，招已经画在空里了」
      //   的同时，红虚线就在眼前。所以贴着 w1 的触发线（860）放，不是提前 470px 放。
      { x: 880, y: 380, w: 70, h: 90, once: true, event: { play: 'c1_t_watch' } },
   ...
   * ══════════════════════════════════════════════════════════════════ */
  {
    ...
    w: 4400, h: 1300,
    // windAx 按 E 的实测标定：稳态漂移 ≈ ax/5，所以 -600 ≈ 120px/s 逆风，
    // 对 240px/s 的走速是「明显被拖住但仍走得动」。原来写 -150（≈30px/s）等于没有风。
    env: { windAx: -600, inkDrainMul: 1.6 },   // 风向左推；雪中墨耗 ×1.6（DESIGN §4 第四回）
   ...
   *     现在的解法：保留 interact（「是他自己动的手」这个动作是这一屏的全部力量），
   *     但把那一架**倒下来横在路上**（blockers）：他要过去就得**搬**。
   *     搬开的那一本正翻在第四回 —— 读是搬完之后躲不掉的结果，不是通行的条件（决议 011）。
   *
   ...
   * ══════════════════════════════════════════════════════════════════ */
  {
    ...
    // ★ 决议 011：因果是「搬」，不是「读」。
    //   「翻一页纸就能挪开一个书架」说不通，玩家会读成「游戏要我读完才放行」——
    //   一旦读出这层，那四屏就从「我撞见的」变成「系统塞给我的」。
    //   所以：书架倒了横在路上，他要过去就得搬；搬开的那一本正翻在第四回，读是搬完躲不掉的结果。
    //   表现层与任何提示都只能表达「搬」，不许出现「阅读」「查看」这类措辞。
    blockers: [
      { x: 3700, w: 26, flag: 'c5_page', note: '倒下的书架横在路上 —— 要过去就得搬开它' }
    ],
    ...
    deco: [
      ...
      { kind: 'shelf', x: 3700, y: 260, fallen: true },  // ★ 就是横在路上那一架（搬开才过得去）
    ...
    triggers: [
      ...
      // ★ 物证：顶层最深处那一架倒在路上。interact ＝ 搬；搬开才过得去（决议 011 / STORY §4.2.1）
      { x: 3620, y: 170, w: 110, h: 90, interact: true, once: true,
